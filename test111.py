...
while loop:
    ...
    if int(user_input) == 1:
        ...
        location = input('Enter location "latitude, longitude, radius" (e.g., 35.95,128.25,1000km):')
        file_name = input('Enter file name (e.g., abc.csv): ')
        ...
        if os.path.isfile(file_location):
            my_csv_file = open(file_name, 'r+', encoding="utf-8")
            ...
            my_csv_file = open(file_name, 'w', encoding="utf-8")
            field_names = ['Index', 'Keyword', 'Tweets']
            writer = csv.DictWriter(my_csv_file, fieldnames=field_names)
            writer.writeheader()
