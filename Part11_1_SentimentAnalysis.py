...
positive_features = [({pos: True for pos in positive_vocab}, 'pos')]
negative_features = [({pos: True for pos in negative_vocab}, 'neg')]
neutral_features = [({pos: True for pos in neutral_vocab}, 'neu')]

train_set = negative_features + positive_features + neutral_features

classifier = NaiveBayesClassifier.train(train_set) # Classifier = module
...
while loop:
    ...
    if int(user_input) == 2: # 임의 입력 sentence 테스트.
        # Predict
        neg = 0
        pos = 0
        neu = 0
        ...
        loop = False
    elif int(user_input) == 1: # 다운받은 파일명으로 실행.
        result = {}
